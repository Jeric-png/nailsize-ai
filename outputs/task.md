...
- [ ] Configure separate development, staging, and production environments.
  - GitHub's manual deployment workflow consumes isolated protected-environment variables/secrets, separate Vercel project IDs, and separate GCS state prefixes. Production additionally requires immutable successful staging deployment/smoke artifacts for the exact commit and model, copies the staging-tested container into the isolated production repository without rebuilding, and proves both repository references resolve to the same digest. The live repository now has exactly `development`, `staging`, and `production`; staging and production accept deployment branches only from `main`. The corrected live audit confirms that every required staging/production variable and `VERCEL_TOKEN` are still absent. GitHub returned HTTP 422 when a required reviewer was configured, explicitly identifying the private-repository billing-plan restriction. The checkbox remains open: no configuration value or secret was guessed, and a supported plan or approved external protection control plus real cross-environment repository grants, OIDC bindings, state buckets, domains, and platform projects is still required.
