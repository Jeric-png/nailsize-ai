...
- [ ] Resolve every critical/high security issue and every severity-1/2 product defect.
  - CI runs a high-severity npm audit and Trivy filesystem scan. Every external GitHub Action is now pinned to an immutable commit, a regression test rejects mutable references, and Dependabot is configured for npm, pip, GitHub Actions, Docker, and Terraform. A read-only repository audit on 2026-07-12 found no open product issues but also found Dependabot alerts, code scanning, secret scanning, and vulnerability alerts unavailable or disabled; branch protection is unavailable for this private repository on its current GitHub plan. This checkbox remains open until those services are enabled and inspected, deployed artifacts are scanned, and the product-defect triage has accountable sign-off.
