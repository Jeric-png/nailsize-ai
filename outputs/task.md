...
- [ ] Configure separate development, staging, and production environments.
  - GitHub's manual deployment workflow consumes isolated protected-environment variables/secrets, separate Vercel project IDs, and separate GCS state prefixes. Production additionally requires immutable successful staging deployment/smoke artifacts for the exact commit and model, revalidates the retained staging Vercel project-control audit against the current team/repository boundary, copies the staging-tested container into the isolated production repository without rebuilding, and proves both repository references resolve to the same digest. On 2026-07-12 the repository was made public with the owner's approval, removing the private-plan restriction. The live repository now has exactly `development`, `staging`, and `production`; staging and production accept deployment branches only from `main`, require one reviewer, prevent self-review, and contain a validated `VERCEL_TOKEN`. Separate `nailsize-ai-staging` and `nailsize-ai-production` Vercel projects are linked to the repository, and their four derived project/team/repository variables are stored in the matching GitHub environments. Neither project has been deployed. The fresh audit still fails closed because 20 required GCP, domain, capacity, monitoring, and billing variables are absent from each deployment environment. The checkbox remains open until real cross-environment repository grants, OIDC bindings, state buckets, domains, platform projects, and reviewed operational values are configured.
...
- [ ] Deploy the frontend so photos post directly to the inference service rather than through a frontend server function.
  - The Vite client already calls the configured inference origin directly. After protected-environment approval and backend apply, deployment automation upserts only the public production `VITE_INFERENCE_API_URL` to the exact Cloud Run load-balancer origin, requires Vercel to confirm that value, creates the Vercel production build from the connected repository and exact Git SHA through the REST API without the Vercel CLI, and then requires that immutable deployment to become `READY` and `PROMOTED`. Deployment smoke fails unless a same-origin built JavaScript module contains both the exact load-balanced API origin and measurement path. Live network inspection in both configured environments remains pending.
...
- [ ] Test current and previous two major browser versions required by `plan.md`.
  - Required CI now runs the complete functional/accessibility E2E flow on current Playwright Chromium with Android emulation, WebKit with iOS emulation, and desktop Chromium, Firefox, and WebKit. A fail-closed aggregate certification CLI additionally requires all 18 consecutive-version results across real iOS Safari, Android Chrome, and desktop Chrome, Edge, Firefox, and Safari; mobile certification must use physical devices. This checkbox remains open until those real physical/hosted runs pass against the deployed release candidate.
- [ ] Complete automated accessibility scans and manual keyboard, VoiceOver, and TalkBack tests.
  - Axe scans cover landing, preparation, capture, accepted quality, processing, results, typed upload error, and expired-session recovery at both approved viewports. Automated keyboard focus/activation covers the primary path and excludes the programmatically triggered hidden file input from tab order. The certification CLI now requires named mobile/desktop scan, manual keyboard, VoiceOver, TalkBack, and accountable review evidence with zero blocking issues. The checkbox remains open until those real manual device reviews pass.
...
- [ ] Verify monitoring, backups, crash reporting, and analytics cannot capture image or result payloads.
  - A fail-closed source audit now allow-lists every production runtime dependency, Terraform resource address/type, and structured-log field; rejects persistent service types and telemetry SDKs; and requires disabled container access logs, native load-balancer metadata logging without optional fields, query-free browser uploads, query-stripping redirects, and a self-only script policy. CI retains a bounded 30-day JSON report with counts and booleans only. A credentialed live Vercel audit now passes for both isolated projects: Web Analytics is disabled, integration and deployment-hook counts are zero, project variables are empty, directory listing and automatic system-variable exposure are disabled, and protected source maps are enabled. The protected deployment repeats that audit before Google authentication and permits only the public release-bound API origin variable. The checkbox remains open until applied GCP logging/IAM and staging termination/failure telemetry are verified.
- [ ] Resolve every critical/high security issue and every severity-1/2 product defect.
  - CI runs a high-severity npm audit, a Trivy filesystem scan, and a Trivy scan of the built inference image before its read-only privacy smoke; none ignores unfixed high/critical findings. The credentialed deployment workflow scans the exact locally tagged staging-built or production-promoted image before any Cloud Run platform apply. Every external GitHub Action is pinned to an immutable commit, a regression test rejects mutable references, and Dependabot covers npm, pip, GitHub Actions, Docker, and Terraform. On 2026-07-12, repository vulnerability alerts and automated security fixes were enabled; the live Dependabot security-alert and product-issue lists were empty. The 13 initial version-update pull requests were reviewed and closed as superseded or outside the verified compatibility bounds; React Refresh 0.5.3 and TypeScript 5.9.3 were adopted after full validation. Dependabot now suppresses unsupported major version-update noise without disabling security updates. After the repository became public, secret scanning and push protection were enabled, CodeQL default setup completed successfully, and `main` protection was enabled with force-pushes and deletion blocked. Fresh CodeQL and CI runs for `c3f038a` passed, and the live code-scanning and secret-scanning alert lists are empty. This checkbox remains open until a real deployed release image passes the deployment scan and product-defect triage has accountable sign-off.
- [ ] Confirm all CI, staging, production smoke, E2E, accessibility, visual, performance, privacy, and model-validation checks are green.
  - The final `nailsize-release-readiness@1` gate now requires the exact approved model, protected-environment audit, staging promotion, production deployment/benchmark/runtime/image/Vercel/smoke reports, client certification, source privacy report, and deployed-control attestations to describe one commit, model checksum, image digest, and host pair. The production promotion verifier now consumes the retained staging Vercel project audit instead of merely uploading it: exact team/repository identity, the isolated staging project, disabled analytics, zero integrations, allowed public variable names, every pass boolean, and exact bounded fields are mandatory before cloud authentication. The gate rejects missing or extra retained fields and revalidates security-sensitive nested environment, benchmark, smoke, browser, and accessibility contracts instead of trusting schema labels. Missing contracts or accountable references remain insufficient; any complete failed or inconsistent evidence blocks release. This checkbox remains open until the real evidence directory returns `release_ready`.
- [ ] Complete the goal evidence ledger with links to every report and deployment revision.
  - The release-readiness report exposes only bounded aggregate identities, failed-check names, and missing reference names so it can be linked safely. The ledger remains open until every real input artifact, production revision, and resulting readiness report is recorded.
- [ ] Obtain final product, nail-tech, privacy/security, and engineering sign-off.
  - Exact sign-off reference fields are mandatory in `release-attestations.json`; blanks produce `insufficient_evidence`, and free-form or private identity fields are rejected. No sign-off has been invented or inferred from automated checks.
