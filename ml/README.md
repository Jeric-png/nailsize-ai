...
Before committing to the four-photo production protocol, run `nailsize-feasibility-report feasibility-study.json --output feasibility-report.json`. The private study bundle must use `nailsize-feasibility-study@1`, contain exactly ten mapped nail observations for each participant under `production-four-photo@1`, include adjudicated physical widths and best-fit sizes, declare the exact skin-tone, curvature, width, and device cohorts, and carry named feasibility, nail-tech, and adequate-sample reviews. The report enforces the 100-participant/1,000-nail minimum, every overall measurement and size gate, cohort MAE and exact-size parity, and participant-clustered 95% intervals. It emits aggregate evidence only. `four_photo_validated` permits continued launch validation; `insufficient_evidence` keeps the study open; a sufficiently powered failure returns `oblique_required` and stops launch until an oblique protocol is separately designed and validated. Images and file paths do not belong in the bundle or report.
