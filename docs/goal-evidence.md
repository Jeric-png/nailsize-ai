...
- The fail-closed contract requires exactly development, staging, and production; an empty development boundary; exact reviewed staging/production variable and secret name sets; at least one required reviewer; `main` as the only deployment branch; and production self-review prevention. Six focused tests cover passing configuration, missing/shadow environments, configuration drift, branch/reviewer drift, development secret leakage, partial API availability, redaction, and source/document synchronization.
- The first authenticated live audit produced `docs/evidence/github-environment-audit-2026-07-12.json` with all three expected environments absent and `passed` false. A later reversible configuration pass created exactly `development`, `staging`, and `production` and restricted staging and production deployment branches to `main`; no deployment, configuration value, or secret was created.
- The refreshed audit still fails closed: staging and production are restricted to `main`, but have zero reviewers, no configured deployment variables, and no `VERCEL_TOKEN`. A required-reviewer configuration request returned HTTP 422 with GitHub's explicit billing-plan restriction, while the branch policies remained intact. The audit's first refresh exposed and corrected a missing `/environments` API path prefix; a regression test now requires the real repository-relative endpoint shape. The environment task remains open until the repository has a plan or approved external control that supports independent production review and real deployment values can be configured and inspected.
- Fresh verification after the live environment pass and endpoint correction passed 451 Python/inference/ML tests at 86.12% measured coverage, 24 web tests, Ruff, formatting, ESLint, TypeScript, API contract drift, the Vercel-compatible production build, and the high-severity npm audit with zero reported vulnerabilities.
- GitHub CI run [29177663152](https://github.com/Jeric-png/nailsize-ai/actions/runs/29177663152) passed all ten jobs for environment-boundary commit `03a20e1`, including 451 Python/inference/ML tests, both browser gates, contracts, security scans, all three Terraform roots, and the runtime-container privacy smoke.
...
## 2026-07-12 annotation-agreement release contract

- `nailsize-annotation-report` consumes an exact versioned private-study bundle plus separate technician mask roots. It requires two independent technicians, safe relative non-pickled `.npy` masks bounded to 25 megapixels and 32 MiB, at least 10% double annotation, and named agreement and adjudication reviews.
- Categorical disagreements, physical-width differences over 0.5 mm, and reviewer-declared boundary disputes require a third-party adjudicator plus a resolution reference. The retained report contains only dataset identity, counts, aggregate Dice/boundary/label/size/kappa/width metrics, disagreement counts, checks, and review references; tests prove it omits image and technician identifiers.
- The exact model release now contains seven files. `annotation-agreement-report.json` must pass, match the model metadata dataset version, contain the exact aggregate field contract, fully adjudicate material disagreements, and preserve exact double-annotation arithmetic. The emitted manifest is `nailsize-model-release@3` and records paired item/participant counts without copying study records.
- Fresh local verification passed Ruff; 330 Python/ML tests at 91.24% measured coverage; 24 web tests; ESLint; TypeScript; contract drift; the Vercel-compatible production build; workflow lint; changed-file formatting; both installed CLI entrypoints; and the high-severity npm audit with zero reported vulnerabilities.
- GitHub CI run [29173439828](https://github.com/Jeric-png/nailsize-ai/actions/runs/29173439828) passed all ten jobs for implementation commit `4b3f9e7`, including 318 dependency-light Python/ML tests at 91.24% coverage, both browser gates, security scans, contracts, all Terraform roots, and the live read-only container privacy smoke. Dedicated Linux Model Tooling run [29173442302](https://github.com/Jeric-png/nailsize-ai/actions/runs/29173442302) installed the pinned PyTorch/TorchVision/ONNX stack and passed all 46 model factory, release bundle, training, and benchmark tests.
- These checks prove the evidence and release boundary, not real inter-annotator agreement. The checklist remains open until collected study annotations produce the aggregate report and accountable reviewers approve it.

## 2026-07-12 physical best-fit size-calibration release contract

- `nailsize-size-calibration-report` consumes exact privacy-safe JSONL observations linking calibrated physical nail width to an adjudicated physical best-fit tip size. It requires at least 200 participant-disjoint people and 2,000 nails by default, the immutable `platform-default@1` chart, a named overall calibration review, and named reviews for every declared adequately sampled curvature cohort.
- The aggregate report measures exact-size, exact-or-adjacent, severe-miss, unmappable, signed tip-margin, and p90 absolute tip-margin outcomes with participant-clustered 95% bootstrap intervals. It fails unless exact sizing is at least 90%, exact-or-adjacent is at least 99%, severe misses are at most 1%, every physical width is mappable, and no reviewed curvature cohort trails the overall exact-size rate by more than five percentage points.
- The model release is now an exact eight-file bundle. `size-calibration-report.json` must pass, match the selected model's locked dataset and holdout counts, use the exact production chart, contain only the declared aggregate schema, preserve bounded confidence intervals, and retain named reviews. The emitted manifest is `nailsize-model-release@4` and records the chart plus aggregate calibration counts without copying participant or nail records.
- Fresh local verification passed 349 Python/inference/ML tests at 91.74% measured coverage, 24 web tests, Ruff, ESLint, TypeScript, API contract drift, workflow lint, formatting, the installed calibration CLI, the Vercel-compatible production build, and the high-severity npm audit with zero reported vulnerabilities.
- GitHub CI run [29173844844](https://github.com/Jeric-png/nailsize-ai/actions/runs/29173844844) passed all ten jobs for implementation commit `f9d8b46`, including 337 dependency-light Python/ML tests at 91.74% coverage, 24 web tests, 18 strict E2E scenarios, 45 Chromium/Firefox/WebKit compatibility scenarios, security scans, contracts, all Terraform roots, and the live read-only container privacy smoke. Dedicated Linux Model Tooling run [29173845794](https://github.com/Jeric-png/nailsize-ai/actions/runs/29173845794) installed the pinned research stack and passed all 64 model factory, selected-checkpoint export, eight-file release, size-calibration, training, and benchmark tests.
- These checks prove a fail-closed calibration evidence mechanism and exact parity with the production size chart. They do not provide the required participant observations, physical best-fit labels, curvature reviews, or a release-ready model. The calibration checklist remains open until real approved study data passes this contract.

## 2026-07-12 approved research-dataset provenance chain

- `nailsize-dataset-provenance-report` now accepts only an exact research training manifest whose rows declare the locked dataset version, `approved_research_study` origin, and `active_research_consent`. It rejects extra fields, duplicate image IDs, participant split leakage, mixed versions, production origin, and inactive consent.
- The checksum-approved report contains only dataset identity, manifest checksum, aggregate record/participant split counts, and named research plus production-exclusion reviews. Tests prove that image IDs, participant IDs, and paths are absent. Training revalidates the manifest against that approval and carries the report and manifest checksums through the restricted checkpoint and selected-model export.
- The final release is now an exact nine-file bundle. It requires the original provenance report, cross-checks its checksum, manifest identity, dataset version, research/consent boundary, positive split counts, and training count against the selected checkpoint, and emits `nailsize-model-release@5`.
- Fresh local verification passed 372 Python/inference/ML tests at 91.12% measured coverage, 87 model-tooling tests, 24 web tests, Ruff, ESLint, TypeScript, API contract drift, workflow lint, formatting, both installed CLI entrypoints, the Vercel-compatible production build, and the high-severity npm audit with zero reported vulnerabilities.
- GitHub CI run [29174230475](https://github.com/Jeric-png/nailsize-ai/actions/runs/29174230475) passed all ten jobs for implementation commit `e140aa6`, including 356 dependency-light Python/ML tests at 91.12% coverage, 24 web tests, 18 strict E2E scenarios, 45 Chromium/Firefox/WebKit compatibility scenarios, security scans, contracts, all Terraform roots, and the live read-only container privacy smoke. Dedicated Linux Model Tooling run [29174233436](https://github.com/Jeric-png/nailsize-ai/actions/runs/29174233436) installed the pinned research stack and passed all 87 model factory, provenance, selected-checkpoint export, nine-file release, size-calibration, training, and benchmark tests.
- These checks prove the source-managed research-only intake and release chain, not the truth of supplied study metadata, applied Cloud Run IAM, deployed telemetry isolation, real consent records, or real model accuracy. The production-training exclusion checkbox therefore remains open pending those external reviews.

## 2026-07-12 public-release holdout lock chain

- `nailsize-holdout-lock` now revalidates every research-manifest participant assignment with the protected split salt before model selection. Its checksum-approved report binds the dataset version, immutable manifest, deterministic split strategy and thresholds, aggregate test counts, named salt version/review, and an identifier-free test-set commitment; it never contains the salt, participant pseudonyms, image pseudonyms, or paths.
- Training requires both the approved provenance and holdout-lock checksums, recomputes the participant splits, trains only on the train partition, and carries the lock identity into the restricted checkpoint. Selected-checkpoint export preserves that identity, and the exact ten-file release requires the original lock report, the same manifest and dataset, public-test access prohibitions, and exact participant/nail count parity with the accuracy report. The emitted release manifest is `nailsize-model-release@6`.
- Fresh local verification passed 385 Python/inference/ML tests at 90.95% measured coverage, 100 pinned-stack model-tooling tests, 24 web tests, 18 strict E2E scenarios, 45 Chromium/Firefox/WebKit compatibility scenarios, Ruff, ESLint, TypeScript, API contract drift, workflow lint, formatting, installed CLI smoke, the Vercel-compatible build, and the high-severity npm audit with zero reported vulnerabilities.
- GitHub CI run [29174601833](https://github.com/Jeric-png/nailsize-ai/actions/runs/29174601833) passed all ten jobs for implementation commit `49dabb3`, including 368 dependency-light Python/ML tests with two platform skips at 90.95% coverage, 24 web tests, 18 strict E2E scenarios, 45 browser-engine scenarios, security scanning, contracts, all Terraform roots, and the live read-only container privacy smoke. Dedicated Linux Model Tooling run [29174601893](https://github.com/Jeric-png/nailsize-ai/actions/runs/29174601893) installed the pinned research stack and passed all 100 model factory, provenance, holdout-lock, selected-checkpoint export, ten-file release, size-calibration, training, and benchmark tests.
- These checks prove the fail-closed mechanism and release linkage, not that a real public holdout has been collected, locked before selection, or independently reviewed. The holdout checklist remains open until the approved participant-disjoint study manifest produces that evidence.

## 2026-07-12 holdout-linked segmentation evaluation chain

- `nailsize-segmentation-report` evaluates fixed `224x160` probability arrays against separate binary ground-truth masks, applies the named validation-selected probability threshold itself, and requires the exact approved public-holdout checksum, identifier-free commitment, participant count, and nail count. Unsafe paths, malformed arrays, empty nails, holdout drift, and public-holdout threshold tuning fail closed.
- The privacy-safe aggregate report binds the dataset, selected model version and SHA-256, holdout lock, threshold-selection review, segmentation review, mean IoU, mean Dice, mean boundary error, conservative p95 boundary error, and participant-clustered 95% confidence intervals. It emits no participant IDs, image IDs, or mask paths.
- The exact release is now an eleven-file `nailsize-model-release@7` bundle. Release verification requires the original segmentation report, exact metric equality with reviewed model metadata, compatible counts and checksums, all declared gates, and a manifest checksum for downstream deployment verification.
- Fresh local verification passed 402 Python/inference/ML tests at 90.46% measured coverage, 118 pinned-stack model-tooling tests, 24 web tests, 18 strict E2E scenarios, 45 Chromium/Firefox/WebKit compatibility scenarios, Ruff, ESLint, TypeScript, API contract drift, workflow lint, formatting, installed CLI smoke, the Vercel-compatible build, and the high-severity npm audit with zero reported vulnerabilities.
- GitHub CI run [29175012602](https://github.com/Jeric-png/nailsize-ai/actions/runs/29175012602) passed all ten jobs for implementation commit `f1e5241`, including 386 dependency-light Python/ML tests with two platform skips at 91% coverage, 24 web tests, 18 strict E2E scenarios, 45 browser-engine scenarios, security scanning, contracts, all Terraform roots, and the live read-only container privacy smoke. Dedicated Linux Model Tooling run [29175014335](https://github.com/Jeric-png/nailsize-ai/actions/runs/29175014335) installed the pinned research stack and passed all 118 model factory, provenance, holdout-lock, segmentation-evaluation, selected-checkpoint export, eleven-file release, size-calibration, training, and benchmark tests.
- These checks prove the evaluation and release evidence mechanism, not real-world segmentation quality. The boundary-error/IoU checklist remains open until the real selected ONNX model is evaluated on the real locked participant-disjoint holdout and accountable reviewers approve the report.

## 2026-07-12 target-runtime ONNX benchmark gate

- `infra/platform` now defines a Gen2 Cloud Run Job using the same environment-specific digest-pinned inference image, selected model version and checksum, 2 vCPU, 4 GiB, one task, one-way parallelism, zero retries, and a 300-second task timeout. The role-less runtime identity and immutable in-image model preserve the existing no-storage boundary.
- The job runs `python -m app.runtime_benchmark` with 20 warmups and 200 measured CPU-provider iterations over the fixed production tensor contract. It rejects synthetic identities, checksum drift, task retries, shape/provider drift, non-finite output, and p50/p95/p99 values above the necessary 2/5/10-second limits. Those limits are safeguards derived from the end-to-end targets, not evidence that complete capture processing meets them.
- Deployment verifies the live v2 Job and Execution resources plus exactly one execution-scoped structured Cloud Logging sample before retaining `nailsize-cloud-run-onnx-benchmark@1`. Production promotion requires the exact passing staging report for the same immutable image/model. The retained report contains only resource/model identifiers, runtime configuration, tensor dimensions, iteration counts, aggregate latency, and booleans—no image, measurement, result, tensor, or model-output data.
- Fresh local verification passed 422 Python/inference/ML tests at 90.50% measured coverage, 24 web tests, 18 strict E2E scenarios, 45 Chromium/Firefox/WebKit compatibility scenarios, Ruff, ESLint, TypeScript, API contract drift, the Vercel-compatible build, privacy and dependency audits, and Terraform 1.15.8 provider validation plus all seven platform tests.
...
- GitHub CI run [29177019440](https://github.com/Jeric-png/nailsize-ai/actions/runs/29177019440) passed all ten jobs for implementation commit `49bbd15`, including 449 Python/inference/ML tests, the final evidence-chain tests, both browser gates, contracts, security scans, all three Terraform roots, and the live read-only runtime-container privacy smoke.
- A subsequent hardening pass removed reliance on schema-version labels alone. The gate now requires the exact retained field set for every upstream artifact and exact nested structures for protected environments, Cloud Run runtime/latency/check contracts, live smoke result vocabulary, all 18 real-browser records, consecutive versions, and accessibility checks. Extra top-level reviewer data or nested response payloads make evidence insufficient rather than passing.
- Fresh local verification after hardening passed 450 Python/inference/ML tests at 86.12% measured coverage, including adversarial minimal/extra/nested-field cases, plus Ruff and formatting across all Python/ML sources.
- GitHub CI run [29177343939](https://github.com/Jeric-png/nailsize-ai/actions/runs/29177343939) passed all ten jobs for hardening commit `693f316`, including the 450-test Python/inference/ML suite, strict release-artifact contract cases, 18 E2E scenarios, Chromium/Firefox/WebKit compatibility, contracts, security scans, all three Terraform roots, and the runtime-container privacy smoke.
