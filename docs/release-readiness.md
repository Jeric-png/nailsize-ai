...
- product documentation identifies `auto-assumed23-single-v0.2.0`, while manifest tests pin the exact model revision/hash, ONNX tensor shapes, runtime hashes, calibration, measurement, quality-profile, and chart versions;
...
- component/E2E tests prove one local photo and one action can reach one best-fit suggestion without selecting a digit, confirming the reference, or editing markers; lower-confidence but usable nail evidence remains non-blocking; rejected photos expose one plain retake path; and copy is text only;
- the beta instructs customers to use a Singapore 50-cent coin, automatically assumes the detected round reference is `23.00 mm`, and never claims to verify its denomination;
...
- current iOS Safari and Android Chrome remain required for release-quality capture/upload, local inference, results, copy, reset, low-memory, and failure-recovery evidence.
...
Also verify that the direct result is materially faster and easier than the guided fallback, including for users who choose one correction or need a retake.
