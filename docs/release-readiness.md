...
- product documentation identifies `auto-assumed23-single-v0.1.0`, while manifest tests pin the exact model revision/hash, ONNX tensor shapes, runtime hashes, calibration, measurement, quality-profile, and chart versions;
...
- component/E2E tests prove one local photo can reach one selected best-fit suggestion for the chosen nail without a second confirmation, uncertain reference evidence requires correction, lower-confidence but usable nail evidence shows a caution with optional correction, rejected photos fail closed, competing boundary sizes are not shown, and copy is text only;
...
- current iOS Safari and Android Chrome remain required for release-quality capture/upload, local inference, results, optional correction, copy, reset, low-memory, and failure-recovery evidence.
