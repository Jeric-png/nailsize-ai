...
1. The browser accepts one JPEG/JFIF, PNG, WebP, HEIC/HEIF, AVIF, GIF, or BMP file no larger than 12 MB and a chosen digit. Detection uses file signatures and bounded header parsing rather than trusting MIME or extension metadata.
...
5. Ambiguous reference geometry asks for one centre tap. A usable nail detection shows its result immediately; two width markers are available only as an optional correction. Results remain in React state until reset, reload, close, or unmount.
...
| A weak mask becomes a recommendation              | Minimum usability checks, visible overlays, plain-language lower-confidence cautions, optional correction, and fail-closed retakes for unusable geometry                                                         | Model/postprocess tests, component/E2E result and correction flows, and physical validation |
