...
The inbound training boundary is also fail closed: every manifest row must identify the approved research-study origin and active research consent, participant split leakage is rejected, and training requires exact approved checksums for aggregate-only provenance and public-holdout lock reports. The lock recomputes every split using a protected salt held outside Git but publishes only aggregate counts and an identifier-free commitment. The selected checkpoint, ONNX export, and ten-file release bundle preserve and cross-check the provenance, manifest, and holdout-lock identities. These controls establish source-managed chain of custody; they cannot prove that a human supplied truthful study metadata or that remote IAM is configured correctly.
