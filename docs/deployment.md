...
11. for production, promotes only that verified deployment, resolves the protected URL directly back to its deployment ID, and confirms it serves the same local artifact digest.
...
- the same locally recorded artifact digest before and after production promotion; and
