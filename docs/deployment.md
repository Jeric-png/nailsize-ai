...
12. promotes only that byte- and runtime-verified production deployment, verifies its generated deployment identity, polls within a monotonic two-minute control-plane deadline until the public URL resolves directly back to the exact deployment ID and commit, then gives public edges a separate bounded minute to serve the same local artifact digest without an authentication bypass.
