...
Deploy staging first. A production dispatch must supply the successful staging workflow run ID for the same commit, model version, and model checksum. Before cloud authentication, the workflow downloads that run's `deployment-evidence-staging` and `deployment-smoke-staging` artifacts and independently verifies the run identity, digest-pinned image, promoted staging Vercel deployment, endpoint hosts, and exact seven passing smoke checks. A different commit/model, failed run, incomplete artifact, or expired artifact prevents production deployment. Promote within the 30-day evidence-retention window.
