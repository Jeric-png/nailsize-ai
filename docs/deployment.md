...
Before Google authentication, `audit_vercel_projects.py` verifies the selected live project has the exact Git repository and build settings, Node 22, directory listing disabled, protected source maps, automatic system-variable exposure disabled, Web Analytics disabled, no deploy hooks or third-party integrations, and no project variable except the optional public `VITE_INFERENCE_API_URL`. The audit records names, counts, and booleans but never variable values, integration identifiers, Git credentials, or the access token. Immediately before creating the immutable deployment, `verify_vercel_deployment.py` upserts `VITE_INFERENCE_API_URL` for the production build to the exact HTTPS origin returned by the applied Cloud Run platform. The deployment stops if Vercel does not confirm that exact public origin, so a build cannot silently fall back to localhost or a stale backend.

Deploy staging first. A production dispatch must supply the successful staging workflow run ID for the same commit, model version, and model checksum. Before cloud authentication, the workflow downloads that run's `deployment-evidence-staging` and `deployment-smoke-staging` artifacts and independently verifies the run identity, digest-pinned image, promoted staging Vercel deployment, endpoint hosts, and exact seven passing smoke checks. It also revalidates the staging Vercel project-audit contract against the current team and GitHub repository identity, requires the exact staging project, and rejects enabled analytics, integrations, unsafe project variables, failed controls, or extra retained fields. A different commit/model, failed run, incomplete artifact, or expired artifact prevents production deployment. Promote within the 30-day evidence-retention window. Production pulls the verified staging image by digest, retags and pushes those exact bytes into the environment-isolated production repository, then fails unless the resolved destination digest is identical. It never rebuilds the production container.
...
The live public repository currently has `development`, `staging`, and `production`, with `main` as the only staging/production deployment branch. Both deployment environments require one reviewer and prevent self-review. Keep deployment values in their matching environment rather than moving them into shared repository variables or secrets. Because the configured reviewer is the repository owner, a production dispatch initiated by that same identity cannot be self-approved; use a separately authorized write-capable dispatcher or replace the reviewer with an independent write-capable collaborator before deployment.
...
After all model, deployment, browser, accessibility, privacy, security, defect-triage, and sign-off evidence exists, assemble the exact aggregate-only directory and run the final gate described in [`release-readiness.md`](release-readiness.md). The `nailsize-release-readiness@1` result checks that every report identifies the same commit, model, image digest, frontend, API, and production revision. Missing evidence remains insufficient and any complete failing evidence blocks launch.
