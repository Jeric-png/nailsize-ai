...
Generate the feasibility decision with `nailsize-feasibility-report private-study-bundle.json --output feasibility-report.json`. The bundle must use `production-four-photo@1`, provide exactly one mapped observation for every nail, and include named feasibility, nail-tech, and adequately sampled cohort reviews. The retained report is aggregate-only. `insufficient_evidence` leaves the study incomplete; `four_photo_validated` requires every dataset, overall, and cohort gate to pass; `oblique_required` means a sufficiently powered four-photo study failed and public launch must stop. The latter does not authorize an invented capture flow: the oblique protocol requires a separate product decision, study design, and validation before implementation.
