...
Apply it once per staging or production project using remote state and a reviewed plan. Then authenticate Docker to the `artifact_repository_host`, push the validated image, resolve its repository digest, and supply that digest-pinned URI to `infra/platform`.

```sh
terraform -chdir=infra/bootstrap init -backend-config=<approved-backend-file>
